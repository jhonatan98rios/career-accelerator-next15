...
Replace the hardcoded token meter with enforceable generation guardrails that reduce AI spam. A generated roadmap is immutable: the next steps unlock only when the current list is fully completed.
...
- [x] AC-2: Roadmap extension is allowed only when the current roadmap is 100% complete; an incomplete roadmap can never be altered or extended.
      Verify: `rg -n "100%|all steps|roadmap.*limit|canGenerateRoadmap|originating insight" src/app/api/roadmap src/app/profile src/components src/models`
- [x] AC-3: The profile UI removes the fake token count and instead shows actionable lock-state messaging with the next unlock time.
      Verify: `rg -n "unlock|disponivel|amanha|pr[oó]xima gera[cç][aã]o|tokens=|credit" src/app/profile src/components`
- [x] AC-4: Blocked generation attempts return clear, non-ambiguous API errors that the client surfaces without silently failing.
      Verify: `rg -n "429|403|locked|limit|unlockAt|Error while generating" src/app/api/insight src/app/api/roadmap src/components`
- [x] AC-5: Generation-limit state is persisted in MongoDB so refreshes, devices, and direct API calls all respect the same policy.
      Verify: `rg -n "lastInsight|lastRoadmap|unlockAt|generationLimit|usage" src/models src/app/api src/lib`
...
- Do not trust client-reported completion eligibility.
...
POST /api/roadmap
- Auth: Bearer Auth0 access token, unchanged.
- Behavior: allow generation only when the roadmap is complete, unless `Profile.skipAiGenerationGuardrails` is `true`.
- Failure shape: include a clear machine-readable limit reason plus unlock context.
...
| `feature::ai-generation-guardrails::ac-2`                      | acceptance | Gate roadmap extension by completion; a generated roadmap is immutable.               |
...
| `code::src/components/roadmap.tsx::RoadmapUpdateButton`        | source     | Client button that requests the next roadmap steps when the current list is complete. |
