...
export function experienceCardComponent(
  ...
  // Date + location
  const dateRange = formatDateRange(exp.startDate, exp.endDate, exp.current, lang);
...
export function educationCardComponent(
  ...
  const dateRange = formatDateRange(edu.startDate, edu.endDate, false, lang);
...
export function projectCardComponent(
  ...
  const dateRange = formatDateRange(proj.startDate, proj.endDate, false, lang);
  if (dateRange) {
    results.push(
      new Paragraph({
        spacing: { before: 0, after: 20 },
...
export function certificationCardComponent(
  ...
  const dateStr = formatDateRange(cert.date, null, false, lang);
...
export function footerComponent(t: Template, lang: string = "pt"): DocxComponent {
  const text = lang === "en" ? "Generated by Career Accelerator" : "Gerado por Career Accelerator";
  return [
    new Paragraph({
      spacing: { before: 400, after: 0 },
      alignment: AlignmentType.CENTER,
      children: [
        run(text, t, { color: t.colors.muted, size: 16, italics: true }),
      ],
    }),
  ];
}
