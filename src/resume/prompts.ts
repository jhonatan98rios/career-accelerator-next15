...
export function getResumeSystemPrompt(userData?: UserData): string {
...
6. **Language field (meta.language):** Use "pt" for Portuguese input, "en" for English input. This field is REQUIRED.
7. **Dates** must be in YYYY-MM format. If only year is known, use YYYY. Current positions should have endDate: null and current: true.
8. **Skill levels** must be one of: beginner, intermediate, advanced, expert — ONLY if explicitly stated.
9. **Language proficiency** must be one of: basic, conversational, professional, native — ONLY if explicitly stated.
10. **Return ONLY the JSON object.** No markdown, no code fences, no explanation. Just the raw JSON.
...
function buildContext(userData?: UserData): string {
  ...
  const lines: string[] = ["## Known User Data (from profile)"];
