...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (roadmap.user_id.toString() !== user.id) {
      await log(LogLevel.ERROR, "POST /roadmap: Roadmap ownership mismatch", { roadmapId, user_id: user.id });
    ...
    if (!insight) {
      await log(LogLevel.ERROR, "POST /roadmap: Insight not found", { roadmapId, insight_id: roadmap.insight_id.toString() });
    ...
    const updatedRoadmap = await CareerRoadmap.findByIdAndUpdate(
      ...
      {
        $set: { 
          correctiveRetryUsedAt: guardrail.reason === "retry" ? new Date() : roadmap.correctiveRetryUsedAt ?? null,
