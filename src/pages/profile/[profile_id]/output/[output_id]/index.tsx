    ...
    <div>
      ...
      <header className="relative h-screen flex items-center justify-center bg-gradient-to-br from-purple-800 to-indigo-600 overflow-hidden">
        ...
        <div className="z-10 text-center px-4">
          <h1
            className="text-5xl md:text-7xl font-extrabold mb-4 animate-fadeInUp-target"
            {...applyAnimationDelay(300)}
          >
            {insight!.hero.title}
          </h1>
          <p
            className="text-xl md:text-2xl mb-8 animate-fadeInUp-target text-gray-200 bo"
            {...applyAnimationDelay(600)}
          >
      ...
      <main className="relative space-y-16">
        <section id="snapshot" className="container mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6 order-2 md:order-1 animate-fadeInUp-target">
              <h2 className="text-3xl font-bold text-purple-400 mt-8">
                {" "}
                {insight!.marketSnapshot.title}{" "}
              </h2>
            ...
            <div className="relative w-full flex justify-center order-1 md:order-2">
              ...
              <div
                className="absolute top-8 left-12 w-64 h-64 bg-gray-900 rounded-lg transform -rotate-3 flex items-center justify-center text-6xl font-extrabold text-white shadow-lg animate-fadeInUp-target"
                {...applyAnimationDelay(500)}
              >
                Jobs
              </div>
            </div>
        ...
        <section id="compensation" className="container mx-auto px-6">
          <h2 className="text-3xl font-bold text-indigo-300 mb-6 text-center animate-fadeInUp-target">
            {" "}
            {insight!.compensation.title}{" "}
          </h2>
          <div
            className="flex flex-wrap justify-center gap-8 animate-fadeInUp-target text-gray-200"
            {...applyAnimationDelay(200)}
          >
        ...
        <section id="global" className="bg-gray-800 py-24">
          <div className="container mx-auto px-6 text-center space-y-8">
            <h2 className="text-4xl font-bold text-indigo-300 animate-fadeInUp-target">
              {" "}
              {insight!.globalOpportunities.title}{" "}
            </h2>
            <p
              className="text-lg max-w-2xl mx-auto animate-fadeInUp-target text-white"
              {...applyAnimationDelay(100)}
            >
            ...
            <div className="flex flex-wrap justify-center gap-6 mt-8 text-gray-200">
              {insight!.globalOpportunities.cards.map((card, index) => (
                <div
                  key={index}
                  className={`w-full sm:w-[calc(33.333%-2rem)] p-6 ${card.bgColor} rounded-2xl shadow-lg hover:-translate-y-2 transition animate-fadeInUp-target`}
                  {...applyAnimationDelay(index * 200)}
                >
        ...
        <section id="bigtech" className="container mx-auto px-6 py-24">
          <h2 className="text-3xl font-bold text-purple-400 mb-6 text-center animate-fadeInUp-target">
            {" "}
            {insight!.bigTechHiring.title}{" "}
          </h2>
          <div
            className="grid md:grid-cols-2 gap-8 animate-fadeInUp-target text-gray-200"
            {...applyAnimationDelay(100)}
          >
            {insight!.bigTechHiring.items.map((item, index) => (
              <div
                key={index}
                className="p-6 bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition"
              >
        ...
        <section id="roadmap" className="container mx-auto px-6 py-24">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {insight!.roadmap.steps.map((step, index) => (
              <div
                key={index}
                className="relative p-8 bg-gradient-to-br from-purple-600 to-indigo-600 text-gray-200 rounded-3xl shadow-2xl hover:scale-105 transition animate-fadeInUp-target"
                {...applyAnimationDelay(index * 200)}
              >
...
export const getStaticProps = (async (context) => {
  const { profile_id, output_id } = (context.params ?? {}) as {
    profile_id?: string;
    output_id?: string;
  };
